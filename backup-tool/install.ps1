param(
    [string]$backupDir = ($env:USERPROFILE | Join-Path -ChildPath "Dropbox" | Join-Path -ChildPath "CorvusSKK-backup")
)

"Register scheduled task to backup ``userdict.txt`` to ``{0}``" -f $backupDir | Write-Host -ForegroundColor Cyan

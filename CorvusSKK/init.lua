...
t[1]で指定したフォーマットで日付変換する

- tの要素数が4つの場合は残りの3つからY年M月D日を計算する。
- tの要素数が3つの場合は残りの2つから「今年のM月D日」を計算する。
- tの要素数が2つの場合は残りの1つから「今月のD日」を計算する。
- tの要素数が1つだけの場合は実行時点の日時を計算する。
...
元号への変換。t[1]が4桁の場合はyyyy0101として、6桁の場合はyyyyMM01として解釈する。
...
一番近い未来の日付を、t[1]で指定したフォーマットに変換する

- tの要素数が3つの場合は残りの2つから「今年のM月D日」を計算する。M月D日が実行時点から見て過去であれば翌年のM月D日を返す。
- tの要素数が2つの場合はt[2]から「今月のD日」を取得する。D日が実行時点から見て過去であれば翌月のD日を返す。
...
一番近い過去の日付を、t[1]で指定したフォーマットに変換する

- tの要素数が3つの場合は残りの2つから「今年のM月D日」を計算する。M月D日が実行時点から見て未来であれば前年のM月D日を返す。
- tの要素数が2つの場合はt[2]から「今月のD日」を取得する。D日が実行時点から見て未来であれば前月のD日を返す。
...
t[1]で指定したフォーマットで、t[2]の数字（月曜日＝1）に対応した曜日に変換する
...
t[1]で指定したフォーマットで、t[2]の数字（月曜日＝1）に対応した「一番近い未来の曜日」に変換する
...
t[1]で指定したフォーマットで、t[2]の数字（月曜日＝1）に対応した「一番近い過去の曜日」に変換する
...
t[1]で指定したフォーマットで、t[2]で指定した「N日後の日付」に変換する
...
t[1]で指定したフォーマットで、t[2]で指定した「N日前の日付」に変換する
...
校数への変換。
1→初（初校）、2→再（再校）とする。
t[2]で何校で校了とするか指定。校了以降は念校、念々校……として、5以上なら数値も付記する。
...
- t[1]が5桁なら `9784641` 始まりとする
- tの要素数が2以上であればt[2]を区切り文字とする。 `-` の場合は `nnn-n-nnn-nnnnn-n` となる。
...
local function from_single_alphabet(s)
	local mapping = {
		{ "a", { "\u{249C}", "\u{24D0}" } },
		{ "b", { "\u{249D}", "\u{24D1}" } },
		{ "c", { "\u{249E}", "\u{24D2}" } },
...
local function skk_search(key, okuri)
	...
	if string.match(key, "^[a-zA-Z]$") then
		ret = ret .. to_skkdict_record(from_single_alphabet(key))
	end
